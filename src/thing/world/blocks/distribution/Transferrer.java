...
public class Transferrer extends Block{
    ...
    public class TransferrerBuild extends Building{
        ...
        public void drawSelect(){
            ...
            Building acceptor = Units.closestBuilding(team, x, y, range, b -> b instanceof AcceptorBuild && ((items.any() && b.acceptItem(this, items.first())) || b.items.total() < (b.block.itemCapacity * content.items().select(i -> !i.hidden).size)));
            if(acceptor != null) Drawf.square(acceptor.x, acceptor.y, acceptor.block.size * tilesize / 2f + 2f, Pal.place);
        }
    }
}
